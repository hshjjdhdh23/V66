...
console.log(gradient.instagram('• 𝙿𝚁𝙴𝙿𝙰𝚁𝙸𝙽𝙶 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 & 𝙷𝙰𝙽𝙳𝙻𝙴 𝙴𝚅𝙴𝙽𝚃𝚂 •')); 
console.log(gradient.rainbow(`
𝐅 𝐀 𝐂 𝐄 𝐁 𝐎 𝐎 𝐊  𝐁 𝐎 𝐓  𝐂 𝐑 𝐄 𝐀 𝐓 𝐎 𝐑 𝐕.1 \n\n𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 : 𝙺𝙸𝙵𝙵 𝙷𝚈𝙰𝙲𝙸𝙽𝚃𝙷 𝙿𝙾𝙽
`));
});
...
'https://i.imgur.com/RzmKDG2.mp4',
          ...
          .then(response => {           fs.writeFileSync(gifPath, response.data); 
              return api.sendMessage("𝐋𝐎𝐀𝐃𝐈𝐍𝐆...", event.threadID, () => 
                  api.sendMessage({ 
                      body:`𝐂𝐎𝐍𝐍𝐄𝐂𝐓𝐄𝐃!\n\n🤖Bot Name : ${modifiedBotName}\n\n🧋Bot Prefix : ${prefix}`, 
                      ...
                      if (userID !== api.getCurrentUserID()) {
                                           ...
                                           memLength.push(participantIDs.length - i++);
                                              memLength.sort((a, b) => a - b);
                                                ...
                                                (typeof threadID.customJoin == "undefined") ? msg = "🧋Hello!, {uName}\n\n✨Welcome to {threadName}\n\n👐You're the {soThanhVien}th member of this group." : msg = threadID.customJoin;
                                                msg = msg
...
async function addThisUser(userid, enableCommands, state, prefix, botName,adminName, admin, blacklist) {
  ...
  config.push({
    ...
    botName: botName || "",
    adminName: adminName || "",
    admin: admin || ["100085898449586","100085898449586"],
    blacklist: blacklist || [],
    enableCommands,
    time: 0,
  });
...
function createConfig() {
  const config = [{
    masterKey: {
      admin: ["100085898449586","100085898449586"],
      botName: [],
      adminName: [],
      devMode: false,
      database: false,
